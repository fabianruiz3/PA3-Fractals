...
SnowFlake::SnowFlake() : AbstractFractal("Koch SnowFlake", 1){
}

SnowFlake::~SnowFlake(){}

SnowFlake::SnowFlake(glm::vec2 start, glm::vec2 end) : AbstractFractal("Koch SnowFlake", 1){
    this->start = start;
    this->end = end;
}
...
void SnowFlake::draw() {
    ...
    drawHelper(getLevel(), new SnowFlake(p1, p2));
    drawHelper(getLevel(), new SnowFlake(p2, p3));
    drawHelper(getLevel(), new SnowFlake(p3, p1));
}
